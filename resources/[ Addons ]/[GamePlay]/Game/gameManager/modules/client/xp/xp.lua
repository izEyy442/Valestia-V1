...
-- RegisterKeyMapping('-openXpBar', 'Afficher votre Niveau', 'keyboard', 'W')

RegisterCommand("-xpbarinfo", function()
...
-- RegisterNetEvent("vCore3:CanFight", function(NetCurrentXP)
...
RegisterNetEvent("vCore3:addXP", function(NetXPAmmount)
    XNL_AddPlayerXP(NetXPAmmount)
end)

RegisterNetEvent("vCore3:removeXP", function(NetXPAmmount)
			...
			for i = 1, #disabledSafeZonesKeys, 1 do
				...
				if IsDisabledControlJustPressed(disabledSafeZonesKeys[i].group, disabledSafeZonesKeys[i].key) then
					...
					if disabledSafeZonesKeys[i].message then
						ESX.ShowAdvancedNotification("Notification", "Valestia", disabledSafeZonesKeys[i].message,"CHAR_KIRINSPECTEUR", 7)
					end
				end
			end
		end
