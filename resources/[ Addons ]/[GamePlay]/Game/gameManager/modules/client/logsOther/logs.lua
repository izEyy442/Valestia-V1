		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'suicide' then
				...
				if debug then
					...
					ShowDebug('Event ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~1~s~\nPlayerID: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~' ..source.. '~s~\nKillerID: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~'.._killer)
					ShowDebug('DeathReason: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~'..DeathReason.. '~s~\nWeapon: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~'.._Weapon)
				end
			else
				TriggerServerEvent('playerDied',2,PlayerId(),GetPlayerServerId(Killer),DeathReason,Weapon)
				if debug then
					if Weapon then _Weapon = Weapon else _Weapon = "N/A" end if Killer then _killer = Killer else _killer = "N/A" end
					ShowDebug('Event ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~2~s~\nPlayerID: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~' ..PlayerId().. '~s~\nKillerID: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~'..GetPlayerServerId(Killer))
					ShowDebug('DeathReason: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~'..DeathReason.. '~s~\nWeapon: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~'.._Weapon)
				end
			end
			Killer = nil
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedShooting(playerped) then
			...
			if debug then
				ShowDebug('Weapon: ~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~' ..WeaponNames[tostring(GetSelectedPedWeapon(playerped))])
			end
...
function ShowDebug(text)
	...
	AddTextComponentSubstringPlayerName("~HUD_COLOUR_PM_WEAPONS_PURCHASABLE~vCore1 Debug:~s~\n"..text)
