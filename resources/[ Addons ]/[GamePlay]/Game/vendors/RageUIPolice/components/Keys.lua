...
Keys.Register = function(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('RageUIPolice-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('RageUIPolice-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
