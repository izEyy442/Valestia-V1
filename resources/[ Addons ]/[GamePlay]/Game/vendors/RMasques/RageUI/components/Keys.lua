...
function Keys.Register(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('RageUIMasques-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('RageUIMasques-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
