...
Keys.Register = function(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('RageUIjh-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('RageUIjh-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            Action();
        end
    end, false)
